...
# =========================================================================
# R2 -- Crisis Hedge Adaptive
# =========================================================================

class CrisisHedgeAdaptive(Strategy):
    """R2-CrisisHedgeAdaptive: dynamic short equity during crises.

    Uses realized-vol percentile as VIX proxy combined with SPY trend
    to size short equity / long safe-haven positions. Flat most of
    the time; profits during sustained stress.
    """

    name = "R2-CrisisHedgeAdaptive"

    _VOL_WINDOW: int = 20
    _SMA_WINDOW: int = 100
    _PCTILE_LOW: float = 0.20   # 20th percentile
    _PCTILE_HIGH: float = 0.80  # 80th percentile
    ...
    def generate_weights(
        ...
        # Realized vol as VIX proxy (annualized)
        rvol = (
            spy.pct_change()
            .fillna(0.0)
            .rolling(self._VOL_WINDOW, min_periods=1)
            .std()
            .fillna(0.0)
            * np.sqrt(252)
        )

        # Expanding percentile rank of realized vol
        rvol_rank = rvol.expanding(min_periods=1).rank(pct=True).fillna(0.0)

        # SPY below 100-day SMA
        sma100 = spy.rolling(self._SMA_WINDOW, min_periods=1).mean()
        below_sma = spy < sma100

        # Conditions
        vol_above_20 = pd.notna(rvol_rank) & (rvol_rank >= self._PCTILE_LOW)
        vol_above_80 = pd.notna(rvol_rank) & (rvol_rank >= self._PCTILE_HIGH)

        # Extreme stress: vol above 80th pctile AND below SMA
        extreme = vol_above_80 & below_sma
        # Moderate stress: vol above 20th pctile AND below SMA (but not extreme)
        moderate = vol_above_20 & below_sma & ~extreme

        if "SPY" in w.columns:
            w.loc[moderate, "SPY"] = -0.30
            w.loc[extreme, "SPY"] = -0.50
        if "TLT" in w.columns:
            w.loc[moderate, "TLT"] = 0.30
            w.loc[extreme, "TLT"] = 0.30
        if "GLD" in w.columns:
            w.loc[moderate, "GLD"] = 0.20
            w.loc[extreme, "GLD"] = 0.20
